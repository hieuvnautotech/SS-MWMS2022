    ...
    if ($("#c_mt_exp").val().trim() == "") {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result == true) {
                    debugger;
                    var id = data.kq.mt_id;
                    $("#WHSCommonGrid").jqGrid('addRowData', id, data.kq, 'first');
...
$("#m_save_but").click(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.result == true) {
                var id = data.kq.mt_id;
                $("#WHSCommonGrid").setRowData(id, data.kq, { background: "#d0e9c6" });
                SuccessAlert(data.message);
            } else {
                ErrorAlert(data.message);
            }
                ...
                success: function (data) {
                  //jQuery("#WHSCommonDtGrid").setGridParam({ rowNum: 20, datatype: "json" }).trigger('reloadGrid');
                    if (data.result == true) {
                        debugger;
                        var id = data.kq.mt_id;
                        $("#WHSCommonDtGrid").jqGrid('addRowData', id, data.kq, 'first');
                        $("#WHSCommonDtGrid").setRowData(id, false, { background: "#d0e9c6" });
                        SuccessAlert(data.message);
                    } else {
                        ErrorAlert(data.message);
                    }
    ...
    if (fun_check2() == true) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.result == true) {
                    var id = data.kq.cdid;
                    $("#WHSCommonDtGrid").setRowData(id, data.kq, { background: "#d0e9c6" });
                    SuccessAlert(data.message);
                } else {
                    ErrorAlert(data.message);
                }
